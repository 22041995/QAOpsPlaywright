...
```ts
  ...
  2  | test('Popup validation',async({page})=>{
> 3  | await page.goto("https://rahulshettyacademy.com/AutomationPractice/");
  ...
  13 | await page.locator("#mousehover").hover();
  14 | const framePages= page.frameLocator("#courses-iframe");
  15 | await framePages.locator("li a[href*='lifetime-access']:visible").click();
  16 | const checkText= await framePages.locator('.text h2').textContent();
  17 | console.log(checkText.split(" ")[1]);
  18 | 
  19 | 
  20 | })
```